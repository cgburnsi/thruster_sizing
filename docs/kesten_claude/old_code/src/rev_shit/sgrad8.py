...
def RHETF(A, B, C, D, E, N):
    """
    Calculates the heterogeneous reaction rate.
    A = CPS (Surface Concentration)
    B = CPX (Local Concentration)
    """
    # --- THIS IS THE FIX ---
    # If concentration (B) is zero or negative, rate is zero.
    if B <= 0.0:
        return 0.0
    
    # If surface concentration (A) is zero, rate is zero.
    if A == 0.0:
        return 0.0
    # --- END OF FIX ---
...
def CPXF(X, Y, Z):
    """
    Calculates the local concentration (CPX) at a normalized radius (X).
    Y = XOA (normalized penetration depth)
    Z = CPS (surface concentration)
    """
    # --- THIS IS THE FIX ---
    # If our position X is inside the zero-concentration core (X < XOA),
    # the concentration is zero. 
    if X <= Y:
        return 0.0
    # --- END OF FIX ---
    
    # If XOA (Y) is 1.0, we get 0/0. Handle this.
    if Y >= 1.0:
        return X * Z # Linear profile from 0 to CPS
...
def CALC_HC(T, G, AP, CI1, CI2, CI3, CI4):
    ...
    CF4 = UNBAR('CFTBL4', T)
    RHO = CI1 + CI2 + CI3 + CI4
    if RHO == 0.0: return 0.0, 0.0, MU # Safety check
...
def temperature_fluctuates(T1, T2, T3):
    # This logic from the FORTRAN code is tricky
    # min(T1, T2, T3) < T2 implies T2 is not the minimum
    # max(T1, T2, T3) > T2 implies T2 is not the maximum
    # So this triggers if T2 is *not* the min *or* *not* the max
    # (i.e., if the temperature is oscillating)
    return min(T1, T2, T3) < T2 or max(T1, T2, T3) > T2

def SGRAD(TEMP, PRES, GG, C1, C2, C3, C4, DIF3, DIF4, A, AP, GMMA, KP, ALPHA2, ALPHA3, EN3):
    MAXITER = 50
    T = TEMP
    P = PRES
    G = GG
    ...
    # Initialize these here for the return statement
    GRAD = 0.0
    TGRAD = 0.0
    ...
    while WAF1 <= 0.95:
        ...
        for LP1 in range(1, MAXITER + 1):
            ...
            if DP3 == 0.0:
                raise ValueError("DP3 (Ammonia Diffusion) is zero.")
            
            DCPDX = KC3 / DP3 * (CI3 - CPS)
            
            # Check for divide by zero
            if DCPDX == 0.0:
                X0_est = A # Set to A if there's no gradient
            else:
                X0_est = A - CPS / DCPDX
            ...
            # --- FIX: Changed 'H4TBL' to 'TBLH4' ---
            H3 = UNBAR('H3TBL', TPS) # Use last loop's TPS
            TPS = T - (UNBAR('TBLH4', T) * KC4 * CI4 + H3 * DP3 * DCPDX) / HC
            TPS = max(TPS, 1.0) # Ensure temp is not zero or negative
            
            # Recalculate properties at new TPS
            H3 = UNBAR('H3TBL', TPS)
            DP3 = DP3F(TPS, D03, P)
            GAMMA = GMMA   # It used to be BGM / TPS which may be incorrect.
            BETA = -CPS * H3 * DP3 / (KP * TPS)
            KO = ALPHA2 * np.exp(-GAMMA) * CI1 ** EN3

            # --- THIS IS THE FIX ---
            # If KO (the reaction constant) is effectively zero due to low
            # temperature, the reaction rate is zero. Set gradients to zero
            # and return immediately, skipping the convergence loop.
            if KO < 1e-30:
                print("   Ammonia decomp. rate is negligible. Skipping SGRAD loop.")
                GRAD = 0.0
                TGRAD = 0.0 # No reaction = no temp change in particle
                return GRAD, TGRAD
            # --- END OF FIX ---
            ...
            if KC3 == 0.0:
                raise ValueError("KC3 (Ammonia Mass Transfer Coeff) is zero.")
            ...
            if temperature_fluctuates(TPS, TPSP, TPSPP):
                ...
                DP3 = DP3F(TPS, D03, P)
                # --- FIX: Changed 'H4TBL' to 'TBLH4' ---
                DCPDX = (HC * (T - TPS) - UNBAR('TBLH4', T) * KC4 * CI4) / (H3 * DP3)
