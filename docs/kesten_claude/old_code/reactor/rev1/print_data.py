...
def print_liquid_data(Z, TEMP, H, DHDZ):
    print(f'Z:{Z:.6e} [ft], TEMP:{TEMP:.2f} [degR], H:{H:.2f} [BTU/lb], DHDZ:{DHDZ:.3e} [ft]')
    
def print_lqvp_data(Z, TEMP, H, WFV):
    print(f'Z:{Z:.6e} [ft], TEMP:{TEMP:.2f} [degR], H:{H:.2f} [BTU/lb], WFV:{WFV:14.5E}')

def print_vapor_data_1(Z, T, P, H, C1, C2, C3, C4):
    print(f'Z:{Z:.6e} [ft], TEMP:{T:.2f} [degR], P:{P:.2f}, H:{H:.2f} [BTU/lb], C1:{C1:.2e}, C2:{C2:.2e}, C3:{C3:.2e}, C4:{C4:.2e}')