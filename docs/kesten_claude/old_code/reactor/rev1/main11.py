...
class SimConfig:
    ...
    LV_MAX     = 10                # [-] Maximum Number of steps in the Liquid-Vapor Zone
...
if __name__ == '__main__':
    ...
    while Z[LL] < ZEND and LL+1 < config.LV_MAX:
        ...
        if KFLAG == 0:
            ...
            if JFLAG == 1:      # =1 means the increment is initially too small
                if abs(DTDZ) * DZ > 0.01 * TEMP:
                    DZ1 = DZ
                    ...
                    H = H + DHDZ[LL] * DZ
                    if H < HV:
                        print('Liquid Hydrazine')
                        break
                    else:
                        pass
                else:
                    # Check if we have reached the end of the injector
                    if (1.0 + DZ / (Z[LL] - Z0) + 0.01 * Z0 / abs(Z[LL] - Z0)) > 0.0:
                        H = H + DHDZ[LL] * DZ
                        if H < HV:
                            print('Liquid Hydrazine - another check point')
                            break
                        else:
                            pass
                    else:
                        ...
                        if H < HV:
                            ...
                            pass
            else:               # JFLAG=0 means DZ increment is ok
