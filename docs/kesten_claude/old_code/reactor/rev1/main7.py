...
def SGRAD(T, P, GG, C1, C2, C3, C4, DIF3, DIF4, A, AP, BGM, KP, ALPHA2, ALPHA3, EN3):
    ...
    CPS = CI3 - A * RIESUM/KC3
        ...
        # Logic fork depending on LTFLG
    if LTFLG != 1:
        # Case 80: First time we're checking low CPS
        if CPS < 0.25 * CI3:
            # Case 81: CPS is too low, flag it and restart loop
            LTFLG = 1
            # Go TO LINE 22 in SGRAD.F code
            ...
            # Case 130: CPS is acceptable, continue
            CMCPN = CI3 - CPS
    else:
        # Case 84: We've already flagged low CPS once, reset flag
        LTFLG = 0
        if CPS < 0.0:
            # Case 89: CPS is negative → clamp to zero and continue
            CPS = 0.0
            # goto_46()  # Recompute DCPDX and continue
            return
        else:
            # Case 130: CPS is non-negative, accept it
            CMCPN = CI3 - CPS
