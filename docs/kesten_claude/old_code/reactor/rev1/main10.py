...
if __name__ == '__main__':
    ...
    # 1: Initialization - Compute CONC, CPs, WMAV, etc
    ...
    while Z[LL] < ZEND and LL+1 < config.LV_MAX:
        ...
        G, GMMA, K, BETA, DPA = PARAM(T[LL], Z[LL], 1, C4, H4, 0, Z0, G0, FC, GATZ0, AGM, BGM, 
                                      ALPHA1, ALPHA2, DIF3, DIF4, P[LL], KP, C1)
        
        # 2: if C4 != 0.0: Compute T4 (hydrazine)
        T4 = 0.0
        if C4 != 0.0:
        ...
        # 3: if C3 != 0.0: Call SGRAD to get T3 (ammonia)
        T3 = 0.0
        if C3 != 0.0:
        ...
        # The following section implements a dynamic zone transition based on finding the peak temperature.
        # KFLAG = 0: Searching for the temperature peak
        # KFLAG = 1: Temperature peak passed; proceed with standard downstream updates
        # 5: if KFLAG == 0 - Check for Slope Peak
        if KFLAG == 0:
            Z1      = -H4/(ENMX3 * DHDZ[LL])
            Z2      = 0.05*(ZEND-Z[LL])
            if DHDZ[LL] * (1.0 - Z1/Z2) > 0:
                ...
                # Label 4060 logic: Peak Reached or invalid slope step
                DZ = Z2
                H += DHDZ[LL] * DZ
                if H < HL:
                    pass # Continue to next iteration of vapor loop (Same as GOTO 22)
                else:
                    KFLAG = 1 # Mark slope peak reached
                    # Then fall through to continue next region of code (also GOTO 22)
        
        # 6: if KFLAG==1 - Compute DTDZ, DWnDZ, DPDZ, DCnDZ
        # ---- Only applies if we are in post-peak zone ----
        if KFLAG == 1:
        
            if JFLAG == 1:
                # Step 93 — Check if slope is steep enough to justify redivision
                if abs(DTDZ) * DZ > 0.01 * T[LL]:
                    # Step 19 — perform REDIVD redivision
                    DZ1 = DZ
                    DZ, NINT, JFLAG, I = REDIVD(DZ1, DTDZ, NINT, JFLAG, I, LL, Z, Z0, T[LL])
                    print(f"\n KOUNT={KOUNT:2d} --- THIS INTERVAL HAS BEEN REDIVIDED {NINT:4d} TIMES")
                else:
                    # Check if we’re at the end of the injector
                    if (1.0 + DZ / (Z[LL] - Z0) + 0.01 * Z0 / abs(Z[LL] - Z0)) > 0.0:
                        # Skip redivision; continue
                        pass
                    else:
                        continue  # Go to next iteration (i.e., label 16 in Fortran)
        
            else:
                # Step 90–98 — handle DZ sizing for convergence
                ...
                if FC <= 0.0:
                    if IFC == 0:
                        continue  # Go to label 16 (continue stepping)

        # === Update enthalpy ===
        H += DHDZ[LL] * DZ
        
        if H < HV:
            break  # corresponds to GO TO 106
        ...
        # === Update temperature and species concentrations ===
        T[LL] += DTDZ * DZ
        P[LL] += DPDZ * DZ
        C4   += DC4DZ * DZ
        C3   += DC3DZ * DZ
        ...
        # === Compute species sum and mole fractions ===
        SUM = C1 / WM1 + C2 / WM2 + C3 / WM3 + C4 / WM4
        if C4 < 0.0:
            SUM -= C4 / WM4
        if C3 < 0.0:
        ...
        # === KFLAG check: see if slope has not peaked yet ===
        if KFLAG == 0:
            if (FRAC3D - PFRC3D) < 0.05:
                pass  # Accept step
            else:
                # Reject step, retry with DZ halved
                H[LL] -= DHDZ[LL] * DZ
                T[LL] -= DTDZ * DZ
                P[LL] -= DPDZ * DZ
                C4 -= DC4DZ * DZ
        ...
        # Step 10: Store slope indicator and clamp negative concentrations
        PFRC3D = FRAC3D
        ...
        # Advance axial location index and update Z
        LL += 1
        Z[LL] = (Z[LL - 1] + DZ)
        
        # Adjust redivision counters
        if JFLAG == 1:
            NINT -= 1
        if NINT == 0:
            JFLAG = 0

        # Step 11: Output data at each axial step
        print(f"\n   Z       TEMP     PRES     H       C1       C2       C3       C4")
        print(f"{Z[LL]:.8e} {T[LL]:.8e} {P[LL]:.8e} {H:.8e} {C1:.8e} {C2:.8e} {C3:.8e} {C4:.8e}")
        
        print("\n   MFRAC1   MFRAC2   MFRAC3   MFRAC4   FRAC3D")
        print(f"{FRAC1:.8e} {FRAC2:.8e} {FRAC3:.8e} {FRAC4:.8e} {FRAC3D:.8e}")
        
        # Step 12: Check if we should exit the vapor loop
        if Z[LL] > ZBOUND or (KOUNT > 15 and JFLAG == 0):
            break  # Exit main vapor loop
        
        # Otherwise, continue
        INJECT = 0
        # Loop naturally returns to top of while True or for-loop


        '''

        # 7: Check DZ sizing (JFLAG, FC conditions)
        JFLAG   = 0             # Set to 1 if DZ step found to be too small
        I       = 0
        NINT    = 0
        ...
        # 9: Update profiles: H, T, P, Cn
        Z[LL+1]      = Z[LL] + DZ
        H[LL+1]      = H[LL] + DHDZ3[LL] * DZ
        T[LL+1]      = T[LL] + DTDZ * DZ if KFLAG == 1 else T[LL]
        P[LL+1]      = P[LL] + DPDZ * DZ if KFLAG == 1 else P[LL]
        DHDZ[LL+1]   = DHDZ3[LL]
        DERIV[LL+1]  = DERIV3[LL]
        ...
        # 10: Recheck slope if KFLAG == 0
        
        # 11: Print results, update Z, LL
        LL += 1
