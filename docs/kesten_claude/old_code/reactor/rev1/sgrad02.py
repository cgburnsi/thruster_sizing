...
def kcf(a, b, c, d, e): return 0.61 * a / b * (c / b * d) ** -0.667 * (a / (e * c)) ** -0.41

def eval1(a, b): return b**3 / 3.0 - a**3 / 3.0

def eval2(a, b): return b**2 / 2.0 - a**2 / 2.0

def unbar(table, T):
    return np.interp(T, np.linspace(300, 2000, len(table)), table)

def dp3f(temp, diff, press):
    return 14.7 * diff / press * (temp / 492.0) ** 1.823 * (1.0 - np.exp(-0.0672 * press * 492.0 / (14.7 * temp)))
...
def CONC(T, P, WM4, WM3, WM2, WM1, R, H, H4, HF):
    TVAP, PRES = T, P
    XV = -(H - HF) / H4
    C4 = (PRES * WM4) / (R * TVAP) * ((1.0 - XV) / (1.0 + XV))
    C3 = (PRES * WM3) / (R * TVAP) * (XV / (1.0 + XV))
    C2 = (PRES * WM2) / (2.0 * R * TVAP) * (XV / (1.0 + XV))
    C1 = (PRES * WM1) / (2.0 * R * TVAP) * (XV / (1.0 + XV))
    return C1, C2, C3, C4

def SGRAD(T, P, C1, C2, C3, C4, ALPHA2, DIF3, DIF4, G, AP, A, KP, BGM, EN3):
