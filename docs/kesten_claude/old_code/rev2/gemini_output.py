...
# Molar gas constant in J/(kmol*K)
# Note: Using J/kmol/K to align with NASA polynomials which 
# are typically on a molar (or kilomolar) basis.
R_GAS = 8314.472

# === Part 2 & 3: Components (Strategy, Context, Data) ===

@dataclass(frozen=True)
class SpeciesData:
    """
    An immutable dataclass to hold the static, definitional data for a species.
    This pattern ensures a species' definition cannot be accidentally changed
    at runtime.[1]
    """
    name: str
    elemental_composition: Dict[str, int]
    molecular_weight: float  # Units: kg/kmol

class AbstractThermoStrategy(ABC):
    """
    The Strategy Interface.[2, 3]
    Defines the abstract API ("contract") that all concrete thermodynamic
...
# === Part 4: The Mixture Class (Composite / Facade) ===
...
class Mixture:
    ...
    """
    def __init__(self, species_list: List):
    ...
    @X.setter
    def X(self, value: Union, List[float], np.ndarray]):
        """
...
class SpeciesFactory:
    ...
    def create_mixture(self, species_names: List[str]) -> Mixture:
        """
...
# === Part 6: Main Simulation Example ===
