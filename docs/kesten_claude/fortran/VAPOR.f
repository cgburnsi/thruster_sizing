	...
	100 FORMAT(109H1 ******************************************** ENTE       290
	   1RING VAPOR REGION    *********************************  //) 		 300
		...
		FRAC3D=(FRAC1/FRAC2-1.)/(3.-FRAC1/FRAC2)							 370
		WRITE(6,4059)														 380
		WRITE(6,4050) Z(LL),TEMP,PRES,H,C1,C2,C3,C4 						 390
		WRITE(6,37)															 400
		WRITE(6,38) FRAC1,FRAC2,FRAC3,FRAC4,FRAC3D							 410
   3990 CALL UNBAR (SHTBL1(1),1,TEMP,0.,CP4,KK) 							 420   
        CALL UNBAR (SHTBL2(1),1,TEMP,0.,CP3,KK) 							 430   
        CALL UNBAR (SHTBL3(1),1,TEMP,0.,CP2,KK) 							 440   
        CALL UNBAR (SHTBL4(1),1,TEMP,0.,CP1,KK) 							 450   
     ...
     31 CALL UNBAR (TBLH3(1),1,TEMP,0.,H3,KK)								 620
		CALL PARAM(TEMP,Z(LL),0,C3,H3,1,G,GMMA,K,DPA,BETA)					 630  (I originally had the 3 and 6 spots turned around, but it is now fixed here)
		IF(C3)13,30,13														 640
...
C		=1 --- SLOPE HAS REACHED FIRST PEAK
     ...
     15 DZ=Z1 																 780 (I think this is DZ=Z1)
   4000 DTDZ=DHDZ(LL)/CAV													 790
		...
		SUMWM=W1/WM1+W2/WM2+W3/WM3+W4/WM4 									 900 (This was all kinds of wrong.  It is fixed now)
		SMDWDZ=DW1DZ/WM1+DW2DZ/WM2+DW3DZ/WM3+DW4DZ/WM4 		 				 910 (I fixed an error on this line as well.)
		DMDZ=-WMAV/SUMWM*SMDWDZ												 920  (The .pdf scanned file is hard to read, but I think this is correct)
		DPDZ = (DELA-1.)/DELA**3*(1.75+75.*VIS*(1.-DELA)/(A*G))*G**2/ 		 930 (I think it is ...(DELA-1.)/DELA..., but it is hard to read)
	   1      (64.4*A*RHO) 													 940
		DPDZ = DPDZ/144.													 950
...
C		CHECK IF WE HAVE REACHED THE END OF THE INJECTOR
		IF((1.+DZ/Z(LL)-Z0)+.01*Z0/ABS(Z(LL)-Z0)).GT.0.)GO TO 16 			1110 (Fixed the parentheses mismatch)
	 19 DZ1=DZ																1120
     ...
     91 FORMAT (//7H KOUNT=I2 ,37H --- THIS INTERVAL HAS BEEN REDIVIDEDI4,  1150
       X6H TIMES)															1160
     16 H=H+DHDZ(LL)*DZ														1170  (I'm pretty sure this is 'H' based stuff, but it is hard to read')
...
C		PERCENT WE RECALCULATE WITH SMALLER DZ INCREMENT
   ...
   4050 FORMAT (1X,E15.8,1X,E15.8,1X,E15.8,1X,E15.8,1X,E15.8,1X,E15.8,1X,E  1600
	 ...
	 38 FORMAT (20X,E15.8,1X,E15.8,1X,E15.8,1X,E15.8,1X,E15.8 ///) 			1660
		...
		INJECT=0															1690
		GO TO 3990															1700
C		SLOPE HAS TURNED NEGATIVE --- NEXT INCREMENTS ARE (1/3)**7, 				(I had to fix a comment here.  It was ...(1.3)**7,... instead of ...(1/3)**7,...)
...
C		TRANSIENT MODEL
		...
		DZZ=ZGAP/8.															1780  (I think this is correct.  hard to read)
		JJ1=LL-1															1790
